...
/**
 * error: TRUE error, FALSE warning
 */
static void ubx_parse_inf(ubx_msg_t *msg)
{
	log_info("ubx_inf: %s", msg->payload_addr);
}
 ...
 */
gboolean ubx_cfg_nav2(U1 maxsv, gboolean allow_alma_nav, gboolean readack)
{
	U1 model = 0x03; /* automobile */
	...
	if (! ubx_issue_cmd(packet, sizeof(packet)))
		return FALSE;
	if (readack && ! ubx_read_ack(&type_cfg_nav2))
		return FALSE;
	return TRUE;
}
